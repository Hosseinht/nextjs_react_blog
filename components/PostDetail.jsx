...
const PostDetail = ({post}) => {
        const getContentFragment = (index, text, obj, type) => {
            ...
            if (obj) {
                ...
                if (obj.href) {
                    modifiedText = (<a className='inline text-blue-500' href={obj.href}  target="_blank" key={obj.index}>{obj.title}</a>);
                }
            ...
            switch (type) {
                ...
                case 'code-block':
                    return <code key={index} className="text-md border  text-gray-700 px-2 font-semibold">{modifiedText.map((item, i) =>
                        <React.Fragment key={i}>{item}</React.Fragment>)}</code>;
        ...
        return (
            <div className="bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8">
                ...
                <div className="px-4 lg:px-0">
                    <div className="flex items-center mb-8 w-full">
                        <div className="flex items-center  mb-4 lg:mb-0 w-full lg:w-auto mr-8">
                            <img src={post.author.photo.url} alt={post.author.name}
                                 className='align-middle rounded-full  object-cover  w-16 h-16'/>
                    ...
                    {/*{post.content.raw.children.map((product) => (*/}
                    {/*    // <div dangerouslySetInnerHTML={{__html: product.html}} />*/}

                    {/*    <RichText key={product} content={product}/>*/}
